...
class Collections {
    ...
    public static <A, R> R foldl(Function2<? super R, ? super A, R> f, R z, Iterable<A> a) {
        R res = z;
        for (A elem : a) {
    ...
    private static <A, R> R foldrRec(Function2<? super A, ? super R, R> f, R z, Iterator<A> it) {
        if (!it.hasNext()) {
            return z;
        }
        A elem = it.next();
    ...
    public static <A, R> R foldr(Function2<? super A, ? super R, R> f, R z, Iterable<A> a) {
